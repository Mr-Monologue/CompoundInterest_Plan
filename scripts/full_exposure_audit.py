...
for i, a in enumerate(codes):
    for b in codes[i+1:]:
        ...
        pair = {
            ...
            "heavy_position_overlap_level": ov.get("heavy_position_overlap_level") or ("unknown" if is_fix else None),
            "heavy_position_status": "TOP10_DATA_UNAVAILABLE_OR_FALLBACK" if is_fix else "ok",
            "top10_overlap_score": ov.get("top10_overlap_score"),
            "industry_overlap_score": None if ind_missing else ov.get("industry_overlap_score"),
...
checks = {"total_items": len(items), "required_fields": ["overlap_status","holding_source","holding_date","is_fixture"],
          ...
          "with_coverage": 0, "with_scope": 0, "with_heavy": 0,
          "with_industry_status": 0, "with_limitation": 0,
...
for i in items:
    ...
    checks["with_common_holdings"] += 1 if "common_holdings" in i else 0
    ...
    checks["with_heavy"] += 1 if "heavy_position_overlap_level" in i else 0
...
v12_fields = checks["v1.2_evidence"]
v12_total = sum(
    checks.get("with_coverage", 0) if f == "coverage_level" else
    checks.get("with_scope", 0) if f == "overlap_scope" else
    checks.get("with_heavy", 0) if f == "heavy_position_overlap_level" else
    checks.get("with_industry_status", 0) if f == "industry_status" else
    checks.get("with_limitation", 0) if f == "limitation" else
    checks.get("with_common_holdings", 0) if f == "common_holdings" else 0
    for f in v12_fields
)
checks["evidence_complete_rate"] = v12_total / max(len(items) * len(v12_fields), 1)
checks["daily_check_status"] = (
    "TODAY_EVIDENCE_INCOMPLETE" if checks["evidence_complete_rate"] < 0.8
    else "OK"
)
